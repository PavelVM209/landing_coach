...
Если у вас возникают проблемы с доступом к странице через поддиректорию `/coaching/`, вы можете использовать специальный скрипт для обновления конфигурации Nginx:

```bash
# Сделать скрипт исполняемым
chmod +x /opt/landing_coach/update_nginx.sh

# Запустить скрипт обновления Nginx
sudo /opt/landing_coach/update_nginx.sh
```

Скрипт `update_nginx.sh` выполняет следующие действия:
- Автоматически ищет файлы конфигурации Nginx на сервере
- Если не находит нужный файл, предлагает выбрать из списка доступных
- Добавляет или обновляет конфигурацию для маршрута `/coaching/`
- Создаёт резервную копию конфигурации перед изменением
- Проверяет корректность конфигурации и перезагружает Nginx

Если скрипт не сработает автоматически, вы также можете вручную добавить содержимое файла `/opt/landing_coach/nginx/fastpassnews_coaching.conf` в соответствующий конфигурационный файл Nginx.
